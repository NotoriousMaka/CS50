...
def buy():
    ...
    if request.method == "POST":
        ...
        if shares == None or not shares.isdigit() or int(shares) <= 0:
        ...
        if cash < total_cost:
            return apology("Not enough cash to buy this stock", 400)
...
def register():
    ...
    if request.method == "POST":
        ...
        if request.form.get("confirmation") == "":
            return apology("Confirmation password not entered.", 400)

        if request.form.get("password") != request.form.get("confirmation"):
            return apology("The password don`t match.", 400)
        ...
        new_rows = db.execute("SELECT * FROM users WHERE username = (?)", request.form.get("username"))
...
def sell():
    ...
    if request.method == "POST":
        ...
        if symbol == None:
            return apology("Symbol not provided", 400)
        elif shares == None or not shares.isdigit() or int(shares) <= 0:
            return apology("Positive integer needed", 400)
        else:
            shares = int(shares)
